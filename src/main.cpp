...
template <class T>
sf::ConvexShape create_polygon(const rev::Layout& layout, const rev::Hex<T>& hex, const sf::Color& fill_color = sf::Color::White)
{
...
int main()
{
    ...
    std::unordered_map<poly_map_t, sf::ConvexShape, HexHash<poly_map_t::crd_t>> polygon_map;
    ...
    while (window.isOpen())
    {
        ...
        else if (event.type == sf::Event::MouseButtonPressed)
        {
            if (event.mouseButton.button == sf::Mouse::Left)
            {
                ...
                if (b)
                    BOOST_LOG_TRIVIAL(debug) << "(hex, poly) pair emplaced at (" <<
                    hex.q() << ", " << hex.r() << ", " << hex.s() << ')';
            }
            else if (event.mouseButton.button == sf::Mouse::Right)
            {
                ...
                if (it != polygon_map.end())
                {
                    ...
                    BOOST_LOG_TRIVIAL(debug) << "(hex, poly) pair erased at (" <<
                        hex.q() << ", " << hex.r() << ", " << hex.s() << ')';
                }
            }
        }
